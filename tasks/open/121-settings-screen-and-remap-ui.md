...
## DoD
- Text size (with a live preview), reduced motion, sound cues, the side palette variant (with the pair shown), volume and mute (the M key moves here and writes the store), mouse sensitivity, invert Y, sneak toggle or hold, gamepad deadzone; a reset to defaults.
