...
A connection that drops without a close frame (Wi-Fi, a lid, a tab that crashed) is terminated by the relay after 15–20 s of unanswered pings (card 46: PING_MS 5 s, DEAD_MS 15 s). Until its `left`, the roster still holds the name with a client, so the fold refuses the same name from the reopened tab as `taken`, and the room screen tells the player "Це ім’я вже зайняте в цій кімнаті. Оберіть інше." about their own name. A player who obeys and types another name joins as a new player: no side until the next prep (ADR 0014; round.ts: an unseated name waits for it), no character until then, while their old frozen character stands for 60 s. The fold's reason is right; the screen's advice is wrong for this case.
