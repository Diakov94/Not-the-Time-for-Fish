...
// One frame of the caller's loop, in real time: the sim steps and its touch claims go out, each step
// moving every copy toward its owner's pose at that step's moment (the frame's `now` less the time still
// unstepped after it), so a frame of two steps moves a copy two steps' worth and leaves it its velocity;
// and every TICK_MS a tick goes out. The next tick is due TICK_MS after the last one was due, not after
