...
const SENSITIVITY = 0.0025; // radians of camera turn per pixel of mouse motion, times the store's sensitivity
...
export type Actions = { grab: () => void; plant: () => void; interact: () => void; perk: () => void; mark: () => void; next: () => void; report: () => void; emote: (n: number) => void };
...
// `perk`, `next` (the teammate a captured cat watches), `report`, `emote` 0-3; a key held down presses
// once, its repeats press nothing. Sneak, as the store says, toggles sneaking for the character `own`
// names now, so a new character starts upright, or is held. Interact is held from its press on (the
// intent's sniff and defuse) and, let go within TAP, is also a tap. The mouse turns the camera by the
// store's sensitivity, and both it and the right stick by its invert Y. The pad's poll is requested before
// the app's frame, so each frame's intent reads that frame's pad; a pad gone lets go of everything it held.
export function listen(canvas: HTMLCanvasElement, own: () => NetId | undefined, act: Actions): Input {
  ...
  (['emote1', 'emote2', 'emote3', 'emote4'] as const).forEach((action, n) => calls.push([action, () => act.emote(n)]));
  const down = (code: string, t: number) => {
    used(code);
    if (does('interact', code)) input.pressedE = t;
    if (does('sneak', code) && settings().sneak === 'toggle') input.sneaking = input.sneaking === own() ? undefined : own();
  ...
  const turn = (dx: number, dy: number) => {
    input.look.yaw -= dx;
    input.look.pitch = Math.min(PITCH_MAX, Math.max(PITCH_MIN, input.look.pitch + (settings().invertY ? -dy : dy)));
  };
  ...
  addEventListener('mousemove', (e) => {
    const k = SENSITIVITY * settings().sensitivity;
    if (document.pointerLockElement === canvas) turn(e.movementX * k, e.movementY * k);
  });
...
export function intent({ keys, look, sneaking }: Input, own: NetId | undefined): Intent {
  ...
  return {
    ...
    sprint: key('sprint') > 0,
    jump: key('jump') > 0,
    sneak: own !== undefined && (settings().sneak === 'hold' ? key('sneak') > 0 : sneaking === own),
    sniff: held,
    defuse: held,
  };
}
