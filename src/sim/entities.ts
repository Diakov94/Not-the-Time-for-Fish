...
// Every kind's body. A character is a capsule its player drives; everything else is a dynamic prop.
// Mines, traps, bags and lures are placeholders until their cards (39, 40, 42), and a `prop` is the
// Prototype's crate until content gives props their shapes (card 26).
const COLLIDER: Record<Kind, () => ColliderDesc> = {
  cat: () => RAPIER.ColliderDesc.capsule(CAPSULE_HALF_HEIGHT, CAPSULE_RADIUS),
  dog: () => RAPIER.ColliderDesc.capsule(CAPSULE_HALF_HEIGHT, CAPSULE_RADIUS),
