...
// here and its owner's snapshots are ignored; the fold still decides who owns it. A prop the table says
// another client holds is not claimed: the fold's first rule refuses it, and in flight it would drop out
// of its carrier's hand here for a round trip. The caller sends the claims. This client's character
// meets a prop copy through the character controller; a dynamic body meets one through the contact graph.
export function touchClaims(sim: Sim): Claim[] {
  ...
  for (const e of touched) {
    ...
    if (isCharacter(e.kind) || simulatedHere(sim, e) || row?.owner === sim.me || row?.held) continue;
