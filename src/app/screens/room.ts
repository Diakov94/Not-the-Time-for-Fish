...
// leaves the hint bar in its place. The app's player-facing text lives in its screens.
export function roomScreen<T extends { session: { sim: Sim } }>(enter: (code: string, name: string) => Promise<T>): Promise<T> {
  paint();
