...
// draws the table's roster as two columns, the cats and the dogs of the first round as the fold's rotation
// will pick them (ADR 0014), each name with its character for that side, and turns clicks into the sim's
// messages: the player's own look per side, and for the host the start. It keeps no roster and decides no
// side; it is redrawn when what it shows changes. The characters are the roster's
...
export function lobbyScreen(room: string, send: (m: Look) => void, start: () => void): (sim: Sim, host: ClientId) => void {
  ...
  return (sim, host) => {
    ...
    const column = (side: Side) =>
      tag('section', {}, tag('h2', { textContent: SIDE[side] }), ...r.roster.filter((p) => dogs.includes(p.name) === (side === 'dog')).map((p) => row(p, side)));
    ...
    screen.replaceChildren(
      ...
      tag('div', { className: 'teams' }, column('cat'), column('dog')),
