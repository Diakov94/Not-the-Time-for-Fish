...
Acceptance **by play**: brings up a room with the headless test clients (`$HEADLESS_GAMES_CMD`, planned in `GAME.md`), drives one scripted scenario at a named client count and reads its numbers: desyncs between clients, FPS under load, fish delivered, mines armed and defused, time to the first grab. **The driver and the judge are split**: this role drives and measures; "is it fun, is losing fair" is answered at the milestone playtest with the friend group (`GAME.md`), or by one Sonnet 5 scenario when the Producer asks for it. **Parallel playtests are capped by Chromium count**: every playtest brings up vite, a Chromium and three to eight clients, so two run in parallel until the load under a wave is measured (the "Batch sizes" section of `STUDIO.md`). One scenario and client count per worker, after integration, not after every batch.
...
**Where the question is in words, the answer comes from a human round.** Instruments do not answer "is it boring", "is losing fair", "is this silence or emptiness"; here those are asked at the friend-group playtest, and this role brings the numbers and the reproduction that make the human answer checkable. What the scenario script could not reach is named in the report, not guessed. The pilot's question to the loser still stands for the humans: "at which moment could you have prevented this?": the answer "I did not know at all" is itself the defect.
