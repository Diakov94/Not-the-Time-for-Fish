...
const scenarios: Record<string, Scenario> = { default: defaultGame };
...
const { values } = parseArgs({
  options: { scenario: { type: 'string', default: 'default' }, clients: { type: 'string', default: '2' }, seconds: { type: 'string', default: '20' } },
});
...
const r = await run(scenario, clients, seconds);
const m = (x: number) => x.toFixed(3).padStart(8);
const f = (x: number, w = 8) => x.toFixed(1).padStart(w);
console.log(`headless: ${values.scenario}, ${clients} clients, ${seconds} s (${scenario.about})`);
...
console.log(`tables at the end: ${r.tablesAgree ? 'deep-equal' : 'NOT equal'} on ${clients} clients; doomed claims ${r.doomed} of ${r.claims}`);
console.log('client   side   ticks/s  min in 1 s  up kB/s  down kB/s  events');
for (const c of r.clients) {
  console.log(`${c.id.padEnd(8)} ${String(c.side).padEnd(4)} ${f(c.ticks)} ${String(c.minTicks).padStart(11)} ${f(c.up)} ${f(c.down, 10)} ${String(c.events).padStart(7)}`);
}
