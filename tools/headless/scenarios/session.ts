...
import { roundsOf } from '../../../src/sim/round.ts';
import { entered, VISIBLE, type Run, type Scenario, type Verdict } from '../run.ts';
import aRound from './round.ts';

const MATCHES = 2;
const ROUNDS = (players: number) => MATCHES * roundsOf(players); // a match is the rotation's rounds (ADR 0014)
...
function judge(r: Run): Verdict {
  ...
  const want = ROUNDS(new Set(r.seats).size);
  ...
  const matches = rounds.filter((ts) => ts[0] && ts[0].round.round === ts[0].round.rounds);
  ...
  const lines = [
    `every over at one message: ${rounds.map((ts, k) => `round ${k + 1} ${one[k] ? `seq ${ts[0]!.seq}` : 'DIFFERENT'}`).join(', ')}; ${n} of ${want} rounds over on ${here.length} clients`,
  ...
  const ok = n === want && one.every(Boolean) && matches.length === MATCHES && same.every(Boolean) && r.desyncs.every((x) => x <= VISIBLE.max) && ticking && r.errors.length === 0;
  return { lines, ok };
}
...
const session: Scenario = {
  ...
  judge,
  seconds: ROUNDS(7) * aRound.seconds!, // 7 and 4 players have the most rounds, 4 a match
  rounds: ROUNDS,
};
