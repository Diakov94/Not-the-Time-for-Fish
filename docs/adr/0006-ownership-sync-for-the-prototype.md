...
- `claim {id, hold}` from `from`: the claim is **rejected** and nothing changes if the entity is held by someone other than `from`, or if `hold` is false and the entity has a home. Otherwise `owner = from`, `held = hold`. So two simultaneous grabs resolve to the first delivered; two pushes resolve to the last delivered, which is the player touching the prop most recently; and a character changes hands only through a grab, never through a bump, so a player keeps control of its own character until a dog grabs it.
...
- `left {id, host}` from the relay: every entity owned by the leaver is released (`held = false`) and goes to its home if the home is still in the room, otherwise to `host`. So a cat a leaving dog carried returns to its own player at once, as GAME.md's disconnect rule asks, a carried prop goes to the host, and the leaver's own character goes to the host and stays as a frozen kinematic body. Removal after 60 s and rejoin are Vertical Slice rules.

Claims are sent on a grab action (`hold: true`) and on the first contact between a body the client owns and a prop it does not (`hold: false`), at most once per prop per 500 ms per client; a client never sends a touch claim for a character, and the fold rejects one anyway. While its claim is in flight, a client simulates the entity it is touching and ignores incoming snapshots for it: the grab feels local; the fold decides. A held entity is a kinematic follower of its carrier on the carrier's client and a kinematic copy everywhere else; a released entity turns dynamic on its new owner. Body types switch with `setBodyType` on the fold's decision, on every client at the same message.
