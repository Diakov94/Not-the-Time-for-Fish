...
| one ping per impact | when two synced bodies of different owners collide and the other is a free moving prop simulated elsewhere, the owner of the lower net id sends; when the other is a character, a held body or a copy at rest, the client that felt the impact sends (amended at batch S1: the literal lower-id rule measured 0 pings for a crate shoved into another client's resting crate); when a synced body hits local debris, that body's owner sends | the rule is the sender's; nothing to reconcile |
